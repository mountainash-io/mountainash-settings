...
This document outlines the process for creating a new release of the mountainash-datacontracts package.
...
1. **Update Version**
   - Navigate to `src/mountainash_datacontracts/__version__.py`
...
4. **Verify Release**
   ...
   - Verify that the following assets are attached to the release:
     - Wheel file (`mountainash_datacontracts-{version}-py3-none-any.whl`)
     - Full SBOM (`mountainash-datacontracts-{version}-sbom-full.xml`)
     - Direct dependencies SBOM (`mountainash-datacontracts-{version}-sbom-direct.xml`)
